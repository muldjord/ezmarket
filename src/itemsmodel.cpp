...
QVariant ItemsModel::data(const QModelIndex &index, int role) const
{
  ...
  if(role == Qt::DisplayRole) {
    ...
    if(index.column() == 0) {
      return ImgTools::getPreparedIcon(allData.icons[allData.items.at(index.row()).icon].pixmap, allData.iconSize, allData.iconBack);
    } else if(index.column() == 1) {
      for(const auto &category: allData.categories) {
        if(category.barcode == allData.items.at(index.row()).category) {
          return ImgTools::getPreparedIcon(allData.icons[category.icon].pixmap, allData.iconSize, allData.iconBack);
        }
      }
    }
